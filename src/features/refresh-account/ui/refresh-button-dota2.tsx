...
import { refreshDota2Action } from "../api/refresh-dota2-action";

interface RefreshButtonProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  accountId: number;
}

export function RefreshButtonDota2({ accountId, ...rest }: RefreshButtonProps) {
  ...
  return (
    <Button
      onClick={async () => {
        setIsLoading(true);
        try {
          const res = await refreshDota2Action(accountId);
          if (res?.message) {
